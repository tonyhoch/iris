...
with col1:
    ...
    st.image(img_setosa, caption='Iris Setosa', width=225)

with col2:
    img_versicolor = Image.open('Iris_versicolor.jpg')
    st.image(img_versicolor, caption='Iris versicolor', width=225)

with col3:
    img_virginica = Image.open('Iris_virginica.jpg')
    st.image(img_virginica, caption='Iris virginica', width=225)
...
if uploaded_file is not None:
    ...
    def user_input_features():
        # get inputs
        sepal_length = st.sidebar.slider('Sepal Length:', min_value=3.0, max_value=10.0, step=0.1)
        sepal_width = st.sidebar.slider('Sepal Width:', min_value=1.0, max_value=5.0, step=0.1)
        petal_length = st.sidebar.slider('Petal Length:', min_value=1.0, max_value=8.0, step=0.1)
        petal_width = st.sidebar.slider('Petal Width:', min_value=0.0, max_value=4.0, step=0.1)
